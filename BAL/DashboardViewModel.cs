    ...
    public class DashboardViewModel
    {
        ...
        public async Task<PendingLeaves> pendingLeaves(int userId)
        {
            try
            {
                ...
                var isProhibitionEmployee = await _context.Employees.Where(x => x.Id == userId).FirstOrDefaultAsync();
                if (isProhibitionEmployee != null && isProhibitionEmployee.FkEmploymentStatus == 2)//means employee is on prohibition
                {
                    leaveData = await _context.Set<PendingLeaves>().FromSqlRaw("dbo.Emp_GetEmployeePendingLeaves_Probation @Userid = {0}", userId).FirstOrDefaultAsync();
                    //leaveData.IsProhibitionEmployee = true;
                }
                else
                {
                    leaveData = await _context.Set<PendingLeaves>().FromSqlRaw("dbo.Emp_GetEmployeePendingLeaves @Userid = {0}", userId).FirstOrDefaultAsync();
        ...
        public async Task<List<EmpLeavesStatus>> empLeaveStatus(int userId)
        {
            try
            {
                List<EmpLeavesStatus> _list = await _context.Set<EmpLeavesStatus>().FromSqlRaw("dbo.Emp_GetEmployeeLeavesStatus @Userid = {0}", userId).ToListAsync();
                return _list;
            }
            catch (Exception ex)
            {
        ...
        public async Task<GetEmployeeLeavesStatus> editLeaves(int LeaveID)
        {
            try
            {
                ...
                var data = await _context.LeaveStatuses.Where(m => m.Id == LeaveID).FirstOrDefaultAsync();
        ...
        public async Task<BalanceLeaveById> editBalanceLeaves(int ID)
        {
            try
            {
                ...
                var data = await _context.BalanceLeaves.Where(m => m.Id == ID).Include(x => x.Emp).Include(x => x.LeaveTypeNavigation).FirstOrDefaultAsync();
                obj.Id = data.Id;
        ...
        public async Task<GetEmployeeLeavesStatus> updateLeavesData(UpdateLeaveData updateLeaveData)
        {
            ...
            {
                GetEmployeeLeavesStatus obj = new GetEmployeeLeavesStatus();
                var data = await _context.LeaveStatuses.Where(x => x.Id == updateLeaveData.LeaveID).FirstOrDefaultAsync();
                //SendMail
                ...
                _context.LeaveStatuses.Update(data);
        ...
        public async Task<BalanceLeaveById> updateBalanceLeavesData(UpdateBalanceLeaveData updateLeaveData)
        {
            ...
            {
                BalanceLeaveById obj = new BalanceLeaveById();
                var data = await _context.BalanceLeaves.Where(x => x.Id == updateLeaveData.ID).FirstOrDefaultAsync();
                data.Balance = updateLeaveData.Balance;
                _context.BalanceLeaves.Update(data);
                _context.SaveChanges();
                return obj;
            }
            catch (Exception ex)
        ...
        public async Task<SaveNewLeaveData> SaveNewLeaveData(SaveNewLeaveData saveNewLeaveData)
        {
            try
            {
                ...
                _context.LeaveStatuses.Add(leaveobj);
                _context.SaveChanges();
        ...
        public async Task<Employee> Login(string UserName, String Password, string AuthCode)
        {
            try
            {
                Employee employee = await _context.Employees.Where(x => x.EmpId == UserName && x.LoginPassword == Password && x.EmpStatus == "Active" && x.Tfmp == AuthCode).FirstOrDefaultAsync();
                return employee;
            }
            catch (Exception ex)
            {
        ...
        public async Task<SingleEmployee> profile(int empId)
        {
            try
            {
                SingleEmployee employee = await _context.Set<SingleEmployee>().FromSqlRaw("dbo.Emp_GetAllEmployeeProfileById @id = {0}", empId).FirstOrDefaultAsync();
                return employee;
            }
            catch (Exception ex)
            {
        ...
        public async Task<List<SingleEmployee>> AllEmployees()
        {
            try
            {
                List<SingleEmployee> _list = await _context.Set<SingleEmployee>().FromSqlRaw("[dbo].[Emp_GetAllEmployeeProfile]").ToListAsync();
                return _list;
            }
            catch (Exception ex)
            {
        ...
        public async Task<List<TopLeaves>> TopLeaves()
        {
            try
            {
                List<TopLeaves> leaves = await _context.Set<TopLeaves>().FromSqlRaw("[dbo].[HRMS_Get_Top5Leave]").ToListAsync();
                return leaves;
            }
            catch (Exception ex)
            {
        ...
        public async Task<List<TopLeavesDept>> TopLeavesByDept()
        {
            try
            {
                List<TopLeavesDept> leaves = await _context.Set<TopLeavesDept>().FromSqlRaw("[dbo].[HRMS_Get_TopLeaveByDepartment]").ToListAsync();
                return leaves;
            }
            catch (Exception ex)
            {
        ...
        public List<DepartmentList> DepartmentList()
        {
            try
            {
                List<DepartmentList> deptList = _spcontext.DepartmentList.FromSqlRaw("[dbo].[Emp_GetAllDepartment]").ToList();
        ...
        public async Task<List<Top5EmployeeByDepartment>> TopDepartmentLeaves(int DepId)
        {
            try
            {
                List<Top5EmployeeByDepartment> topleaves = await _context.Set<Top5EmployeeByDepartment>().FromSqlRaw("[dbo].[HRMS_Top5EmployeeByDepartmentId] @DepId = {0}", DepId).ToListAsync();
                return topleaves;
            }
            catch (Exception ex)
            {
        ...
        public async Task<List<TotalLeaveByDepartment>> TotalDeptLeaves()
        {
            try
            {
                List<TotalLeaveByDepartment> leaves = await _context.Set<TotalLeaveByDepartment>().FromSqlRaw("[dbo].[HRMS_GetTotalLeaveByDepartment]").ToListAsync();
                return leaves;
            }
            catch (Exception ex)
            {
        ...
        public PendingLeaves pendingLeavesDetails(int userId)
        {
            ...
            {
                //dbo.SomeSproc @Id = {0}, @Name = {1}", 45, "Ada"
                PendingLeaves leaveData = _context.Set<PendingLeaves>().FromSqlRaw("dbo.Emp_GetEmployeePendingLeaves @Userid = {0}", userId).FirstOrDefault();
                return leaveData;
        ...
        public async Task<List<EmployeesLeaveDetails>> employeesLeaveDetailsArea(int DeptID, int EmpID, string FromDate, string ToDate)
        {
            ...
            try
            {
                List<EmployeesLeaveDetails> empleaveData = await _context.Set<EmployeesLeaveDetails>().FromSqlRaw("[dbo].[HRMS_GetEmployeesLeaveDetails] @EmpID = {0},@DeptId = {1}, @fromdate = {2}, @todate = {3}", EmpID, DeptID, FromDate, ToDate).ToListAsync();
                return empleaveData;
            }
            catch (Exception ex)
            {
        ...
        public async Task<List<EmployeesLeaveDetails>> employeesLeaveDetailsExportMonthlyLeaves(int DeptID, int EmpID, string FromDate, string ToDate)
        {
            try
            {
                List<EmployeesLeaveDetails> empleaveData = await _context.Set<EmployeesLeaveDetails>().FromSqlRaw("[dbo].[HRMS_GetEmployeesLeaveDetails_MonthReport] @EmpID = {0},@DeptId = {1}, @FromDate = {2}, @ToDate = {3}", EmpID, DeptID, FromDate, ToDate).ToListAsync();
                return empleaveData;
            }
            catch (Exception ex)
            {
        ...
        public async Task<GetUserImagePath> ImagePath(int empId)
        {
            try
            {
                GetUserImagePath employee = await _context.Set<GetUserImagePath>().FromSqlRaw("dbo.Emp_GetUserImagePath @userid = {0}", empId).FirstOrDefaultAsync();
                return employee;
        ...
        public List<EmployeeList> EmployeeList(int depID)
        {
            try
            {
                List<EmployeeList> empList = _context.Set<EmployeeList>().FromSqlRaw("[dbo].[GetEmployeeList] @Deptid = {0}", depID).ToList();
                return empList;
            }
            catch (Exception ex)
            {
        ...
        public async Task<FirstLineManager> GetFirstLineManager(string emp_id)
        {
            try
            {
                FirstLineManager firstLineManager = await _context.Set<FirstLineManager>().FromSqlRaw("dbo.HRMS_GetReportingManagerbyEmpid @Emp_id = {0}", emp_id).FirstOrDefaultAsync();
        ...
        public async Task<SecondLineManager> GetSecondLineManager(string emp_id)
        {
            try
            {
                SecondLineManager secondLineManager = await _context.Set<SecondLineManager>().FromSqlRaw("dbo.HRMS_GetReportingManagerbyEmpid @Emp_id = {0}", emp_id).FirstOrDefaultAsync();
        ...
        public async Task<ManagerDetails> GetManagerDetails(int id)
        {
            try
            {
                ManagerDetails ManagerDetails = await _context.Set<ManagerDetails>().FromSqlRaw("dbo.HRMS_GetDepartmentManager @ID = {0}", id).FirstOrDefaultAsync();
        ...
        public async Task<List<EmployeeList>> EmployeeListDet(int depID)
        {
            try
            {
                List<EmployeeList> empList = await _context.Set<EmployeeList>().FromSqlRaw("[dbo].[GetEmployeeList] @Deptid = {0}", depID).ToListAsync();
                return empList;
            }
            catch (Exception ex)
            {
        ...
        public string GetHrName()
        {
            try
            {
                ...
                var _role = _context.EmployeeRoles.FirstOrDefault(c => c.FkRole == 5);
                if (_role != null)
                {
                    var employee = _context.Employees.FirstOrDefault(c => c.Id == _role.FkEmployee);
                    if (employee != null)
                    {
        ...
        public async Task<List<GetEmployeeLeavesStatus>> LeaveHistoryByID(int Userid, int fkLeaveType, int lwp)
        {
            try
            {
                List<GetEmployeeLeavesStatus> employeeLeaveDetail = await _context.Set<GetEmployeeLeavesStatus>().FromSqlRaw("dbo.[Emp_GetEmployeeLeaves_History]  @Userid = {0},@fkLeaveType = {1},@lwp = {2}", Userid, fkLeaveType, lwp).ToListAsync();
        ...
        public async Task<List<CustomLeaveDetail>> employeesCustomLeaveDetailsArea(DateTime? StartDate, DateTime? EndDate, int? DeptId, int? EmpID)
        {
            ...
            try
            {
                ...
                {
                    empleaveData = await _context.Set<CustomLeaveDetail>().FromSqlRaw("[dbo].[HRMS_GetEmployeesLeaveDetails_Coustom_Report] @from = {0},@to = {1}, @DeptID = {2}", null, null, DeptId).ToListAsync();
                }
                else
                {
                    empleaveData = await _context.Set<CustomLeaveDetail>().FromSqlRaw("[dbo].[HRMS_GetEmployeesLeaveDetails_Coustom_Report] @from = {0},@to = {1}, @DeptID = {2}", StartDate, EndDate, DeptId).ToListAsync();
                }
                return empleaveData;
            }
            catch (Exception ex)
        ...
        public async Task<CalculateLeavesCount> SummarisedLeavesCounts(int userId, int fkLeaveType, int lwp)
        {
            try
            {
                ...
                leaveData = await _context.Set<CalculateLeavesCount>().FromSqlRaw("dbo.[usp_Total_CalculateLeaves] @UserId = {0},@fkLeaveType = {1},@lwp = {2}", userId, fkLeaveType, lwp).FirstOrDefaultAsync();
        ...
        public CalculateLeavesTakenByYear LeavesCountsPerYearByID(int userId, int fkLeaveType, int lwp)
        {
            try
            {
                ...
                leavePerYearData = _context.Set<CalculateLeavesTakenByYear>().FromSqlRaw("dbo.[usp_CalculateLeavesTaken_By_Year] @Userid = {0},@fkLeaveType = {1},@lwp = {2}", userId, fkLeaveType, lwp).FirstOrDefault();
        ...
        public async Task<List<GetEmployeeLeavesHistory_LWP>> LeaveHistoryLWPByID(int Userid, int lwp)
        {
            try
            {
                List<GetEmployeeLeavesHistory_LWP> employeeLeaveDetail = await _context.Set<GetEmployeeLeavesHistory_LWP>().FromSqlRaw("dbo.[Emp_GetEmployeeLeaves_History_LWP]  @Userid = {0},@lwp = {1}", Userid, lwp).ToListAsync();
                return employeeLeaveDetail;
            }
            catch (Exception ex)
            {
        ...
        public async Task<List<BalanceLeaveById>> BalanceLeaveByID(int Userid)
        {
            try
            {
                var employeeLeaveDetail = from bal in _context.BalanceLeaves.Where(x => x.EmpId == Userid && x.IsDeleted==false).OrderByDescending(x=>x.CreatedOn)
        ...
        public CalculateLeavesTakenLWPByYear LeavesCountsPerYearLWPByID(int userId, int fkLeaveType, int lwp)
        {
            try
            {
                ...
                leavePerYearData = _context.Set<CalculateLeavesTakenLWPByYear>().FromSqlRaw("dbo.[usp_CalculateLeavesTaken_By_Year] @Userid = {0},@fkLeaveType = {1},@lwp = {2}", userId, fkLeaveType, lwp).FirstOrDefault();
                return leavePerYearData;
            }
            catch (Exception ex)
            {
        ...
        public async Task<ClsEmp_GetEmployeePendingLeaves> EmpLeaveTakenAndLeftDetails(int userId)
        {
            try
            {
                ...
                clsEmpLeaveTakenAndLeftDetails = await _context.Set<ClsEmp_GetEmployeePendingLeaves>().FromSqlRaw("dbo.[Emp_GetEmployeePendingLeaves] @Userid = {0}", userId).FirstOrDefaultAsync();
        ...
        public async Task<Emp_ProbationStatusWithColName> Emp_ProbationStatus(int emp_id)
        {
            try
            {
                Emp_ProbationStatusWithColName Provisionstatus = await _context.Set<Emp_ProbationStatusWithColName>().FromSqlRaw("[dbo].[Emp_ProbationStatus_withcolname] @Userid = {0}", emp_id).FirstOrDefaultAsync();
        ...
        public async Task<AppVersion> GetAppVersionInfo()
        {
            try
            {
                var appVersion = await _context.MobileAppUpdateTables.FirstOrDefaultAsync();
        ...
        public async Task<List<EmployeeList_withCode>> EmployeeListWithCodeGenerator()
        {
            try
            {
                List<EmployeeList_withCode> empList = await _context.Set<EmployeeList_withCode>().FromSqlRaw("[dbo].[GetEmployeeRandomCode]").ToListAsync();
                return empList;
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                if (userID == 0)
                {
                    ...
                    foreach (var item in userList)
                    {
                        ...
                        emp = await _context.Employees.Where(x => x.Id == item.UserID).FirstOrDefaultAsync();
                        //SendMail
                        isMailSend = true;
                        //isMailSend = await sendmail.sendEmail(emp.OfficialEmail, pass, emp.FullName);
                        emp.Tfmp = pass;
                ...
                else
                {
                    Employee emp = new Employee();
                    string pass = generator.RandomPassword();
                    emp = await _context.Employees.Where(x => x.Id == userID).FirstOrDefaultAsync();
                    emp.Tfmp = pass;
                    //sendMail
                    isMailSend = await sendmail.sendEmail(emp.OfficialEmail, pass, emp.FullName);
                    _context.Employees.Update(emp);
        ...
        public async Task<bool> GetProhbation(int userID)
        {
            ...
            try
            {
                var appVersion = await _context.Employees.Where(x => x.Id == userID && x.FkEmploymentStatus == 2).FirstOrDefaultAsync();
                if (appVersion != null)
                {
                    Is_Prohbition = true;
                }
        ...
        public async Task<ProhbitionEmployeeList> ProhbitionEmployeeLeaves(int UserID)
        {
            try
            {
                ProhbitionEmployeeList empList = await _context.Set<ProhbitionEmployeeList>().FromSqlRaw("[dbo].[Emp_GetEmployeePendingLeaves_Probation] @Userid = {0}", UserID).FirstOrDefaultAsync();
                return empList;
            }
            catch (Exception ex)
            {
        ...
        public async Task<List<LateComing_Details>> LateComingDetails(string StartDate, string EndDate)
        {
            try
            {
                List<LateComing_Details> empList = await _context.Set<LateComing_Details>().FromSqlRaw("[dbo].[Emp_LateComing] @fromdate={0}, @todate={1}", StartDate, EndDate).ToListAsync();
                return empList;
            }
            catch (Exception ex)
            {
        ...
        public async Task<List<LateComing_Details_History>> LateComing_Details_History(string StartDate, string EndDate, int? UserID, int returnList)
        {
            try
            {
                ...
                List<LateComing_Details_History> empList = await _context.Set<LateComing_Details_History>().FromSqlRaw("[dbo].[Emp_LateComing_Details_History]@Userid = {0}, @fromdate={1}, @todate={2},@return={3}", UserID, StartDate, EndDate, returnList).ToListAsync();
                return empList;
            }
        ...
        public async Task<List<LessWorkingHours_Details>> LessWorkingHour_Details(string StartDate, string EndDate)
        {
            try
            {
                List<LessWorkingHours_Details> empList = await _context.Set<LessWorkingHours_Details>().FromSqlRaw("[dbo].[Emp_LateComing_Details] @fromdate={0}, @todate={1}", StartDate, EndDate).ToListAsync();
                return empList;
            }
            catch (Exception ex)
            {
        ...
        public async Task<List<WorkingHoursDetails>> EmpWorkingHoursDetail(string StartDate, string EndDate, int? UserID, int returnList)
        {
            try
            {
                ...
                List<WorkingHoursDetails> empList = await _context.Set<WorkingHoursDetails>().FromSqlRaw("[dbo].[Emp_LateComing_Details_History]@Userid = {0}, @fromdate={1}, @todate={2},@return={3}", UserID, StartDate, EndDate, returnList).ToListAsync();
                return empList;
            }
